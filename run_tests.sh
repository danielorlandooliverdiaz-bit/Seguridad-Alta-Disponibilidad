...
# Crear un directorio de prueba limpio
rm -rf test_environment
mkdir test_environment
cd test_environment

echo "Directorio de prueba 'test_environment/' creado."
...
# Se usa ../main.sh porque estamos dentro del directorio de prueba
...
if diff -q test_file.txt decrypted_symmetric_result.txt && diff -q test_file.txt decrypted_hybrid_result.txt; then
  ...
  # Volver al directorio original antes de salir con error
  cd ..
  exit 1
fi

# --- 6. PAUSA PARA INSPECCIÓN ---
echo "
-----------------------------------------------------------------------------------"
echo "¡PAUSA! El script ha finalizado las operaciones."
echo "Puedes inspeccionar los archivos generados en la carpeta 'test_environment'."
echo "Encontrarás: claves, archivos cifrados, y los resultados descifrados."
read -p "Presiona [Enter] para continuar y limpiar todos los archivos de prueba..."

# --- 7. LIMPIEZA ---
echo "
--- Paso 8: Limpiando archivos de prueba... ---"
# Volver al directorio original y eliminar la carpeta de prueba
cd ..
rm -rf test_environment
echo "Directorio 'test_environment' y su contenido han sido eliminados."
